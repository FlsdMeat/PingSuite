...
export default async function FullLineGraph(graphType,organization){
    ...
    let data = {}
    await axios.get('http://localhost:8080/api/pingResults/allDates')
    .then(async res=>{
        if(res.data !== false){
            Object.keys(res.data).forEach(item=>{
                res.data[item]['color'] = getRandomColor()
            })
            data = await findGraphPoints(res.data, graphType, organization)
            return data
        }
    })
